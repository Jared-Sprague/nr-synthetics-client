...
module.exports = function ({ apiKey = "" }) {
  ...
  async function getAllAlertPolicies() {
    const alertPolicies = await _restAPI.getAlertPolicies();

    return alertPolicies;
  }

  async function getAllAlertChannels() {
    const alertChannels = await _restAPI.getAlertChannels();

    return alertChannels;
  }

  async function getAllAlertConditions(policies) {
    const alertConditions = await _restAPI.getAlertConditions(policies);
  ...
  async function getAllMonitors({ saveConfig = false } = {}) {
    ...
    _props.monitorConfigs.forEach((monitor) => {
  ...
  function _injectAlertRecipients() {
