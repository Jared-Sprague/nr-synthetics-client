...
module.exports = function ({ apiKey = "" }) {
  ...
  async function getAllAlertPolicies() {
    _props.alertPolicies = await _restAPI.getAlertPolicies();

    return _props.alertPolicies;
  }

  async function getAllAlertChannels() {
    _props.alertChannels = await _restAPI.getAlertChannels();

    return _props.alertChannels;
  }

  async function getAllAlertConditions(policies) {
    _props.alertConditions = await _restAPI.getAlertConditions(policies);

    return _props.alertConditions;
  }
  ...
  async function getAllMonitors({ saveConfig = false } = {}) {
    ...
    // Iterate over monitors and inject custom attributes
    _props.monitorConfigs.monitors.forEach((monitor) => {

      // Sanitize monitor name
      monitor.name = monitor.name.replace(/[\/]/gi, ' ');

      // inject scriptFile name
      if (monitor.type === 'SCRIPT_BROWSER' || monitor.type === 'SCRIPT_API') {
        monitor.scriptFile = "'" + monitor.name + ".js'";
      }

      // Inject alert threshold
      monitor.alertThreshold = "2 or more locations failing";

      // Inject the alert recipients
      monitor.alertRecipients = _getAlertRecipients(monitor);
    })
  ...
  function _getAlertRecipients(monitor) {
    const monitor_id = monitor.id;
    let matchingPolicyIds = [];
    let alertRecipients = {};

    // console.log("_getAlertRecipients for monitor Id: ", monitor_id);

    // Find all the alert policies that have this monitor as a condition
    let policy_ids = Object.keys(_props.alertConditions);
    for (let i = 0; i < policy_ids.length; i++) {
      let policy_id = policy_ids[i];
      let conditionGroup = _props.alertConditions[policy_id];
      let matchFound = false;

      // console.log("Looking for monitor match in conditions for policy: ", policy_id, conditionGroup.policy_name)

      // Check the single location conditions
      for (let j = 0; j < conditionGroup.singleLocation.length; j++) {
        let singleLocationCond = conditionGroup.singleLocation[j];
        if (monitor_id === singleLocationCond.monitor_id) {
          console.log("Single location condition match found for Policy ID: ", policy_id, monitor_id);
          matchingPolicyIds.push(policy_id);
          matchFound = true;
          break;
        }
      }
      ...
      if (!matchFound) {
        // Check if there any any multiple location conditions that match this monitor for this policy
        for (let j = 0; j < conditionGroup.multiLocation.length; j++) {
          let multiLocationCond = conditionGroup.multiLocation[j];
          if (monitor_id === multiLocationCond.entities[0]) {
            console.log("Multiple location condition match found for Policy ID: ", policy_id, monitor_id);
            matchingPolicyIds.push(policy_id);
            matchFound = true;
            break;
          }
    ...
    // Get the notification channels that are associated with each matching policy IDs for this monitor
    matchingPolicyIds.forEach((policyId) => {
      _props.alertChannels.forEach((channel) => {
        if (channel.links.policy_ids.includes(parseInt(policyId))) {
          // console.log("Notification channel match found:", policyId, channel.name);
          let channelValue;
          if (channel.type === 'pagerduty') {
            channelValue = 'PagerDuty - ' + channel.name;
          }
          else if (channel.type === 'slack') {
            channelValue = 'SLACK - ' + channel.name;
          }
          else {
            channelValue = channel.name;
          }
          alertRecipients[channel.id] = channelValue;
        }
      })
    });

    // Flatten the alert recipients into a string
    const channelValues = Object.values(alertRecipients).join(', ');
  ...
  async function saveScripts(monitors) {
    ...
    for ( let i = 0, monitor = monitors[i];  i < monitors.length;  monitor = monitors[++i] ) {
      ...
      try {
        ...
        // TODO: Write this log to a file
        console.log( `saveScripts(): Failed writing script [${name}] id=[${id}] type=[${type}]`, error);
      }
    }
  }
